...
import { Heart, Search, MessageCircle, User, Rss, Sun, Moon } from "lucide-react";
...
export default function Layout() {
  ...
  const isPublicRoute =
    ...
    normalizedPath.startsWith("/termsofuse");
  ...
  return (
    <div className="min-h-screen bg-background font-body flex flex-col">
      ...
      {/* CONTENT */}
      <main className="flex-1 pb-28 md:pb-0 md:pl-[88px] relative z-10">
        <Outlet />
      </main>

      {/* TERMS LINK (volitelně skrytý na terms stránce) */}
      {!isPublicRoute && (
        <div className="text-center pb-20 md:pb-4 relative z-10">
          <Link 
            to="/termsofuse" 
                ...
                >
                  <div className={`relative ${isActive ? "animate-float" : ""}`}>
                    <Icon
                      className={`w-5 h-5 ${
                        isActive
                          ? "drop-shadow-[0_0_8px_rgba(218,165,32,0.9)]"
                          : ""
                      }`}
                    />
                    {isActive && (
