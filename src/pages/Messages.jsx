...
export default function Messages() {
  ...
  return (
    <div className="min-h-screen max-w-lg mx-auto p-4">
      <h1 className="text-2xl font-heading font-bold mb-6">Zprávy</h1>
      {matches.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mb-4">
            <MessageCircle className="w-8 h-8 text-primary" />
          </div>
          <h3 className="font-heading font-semibold text-lg mb-1">Zatím žádné konverzace</h3>
          <p className="text-muted-foreground text-sm max-w-xs">Shodněte se s někým a začněte chatovat!</p>
        </div>
      ) : (
              ...
              <Link key={match.id} to={`/chat/${match.id}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-accent/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    {match.last_message_at && <span className="text-[10px] text-muted-foreground">{format(new Date(match.last_message_at), "d. M.")}</span>}
                  </div>
                  <p className="text-xs text-muted-foreground truncate mt-0.5">{match.last_message || "Pozdravte se! 👋"}</p>
                </div>
              </Link>
            );
          })}
